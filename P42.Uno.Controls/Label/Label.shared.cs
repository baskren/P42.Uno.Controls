...
public partial class Label : UserControl
{
    ...
    private void OnTextDecorationsChanged() => _textBlock.TextDecorations = TextDecorations;
    ...
    private void OnLabelAutoFitChanged() { InvalidateMeasure(); }
    ...
    private void OnMinFontSizeChanged()
    {
    ...
    private void OnVerticalTextAlignmentChanged()
    {
    ...
    private void Register(DependencyProperty property, DependencyPropertyChangedCallback callback)
        => PropertyChangedCallbackTokens.Add(property, RegisterPropertyChangedCallback(property, callback));

    private void Unregister(DependencyProperty property)
    ...
    private void OnLoading(FrameworkElement sender, object args)
    {
        ...
        Register(CharacterSpacingProperty, OnCharacterSpacingChanged);
        // ContentStart : Not implemented in Uno
        // ContentEnd : Not implemented in Uno
        Register(FontFamilyProperty, OnFontFamilyChanged);
        Register(FontSizeProperty, OnFontSizeChanged);
        ...
        // FontWeight : above
        // Foreground : above
        Register(HorizontalContentAlignmentProperty, OnHorizontalContentAlignmentChanged);
        ...
        Register(VerticalContentAlignmentProperty, OnVerticalContentAlignmentChanged);
    ...
    private double BoundFontSize(double fontSize)
    {
    ...
    private double ZeroLinesFit(TextBlock control, double min, double max, double availWidth, double availHeight)
    {
        if (control is null)
    ...
    private double WidthAndLinesFit(TextBlock control, int lines, double min, double max, double availWidth)
    {
        /*
        if (control is null)
            return Math.Min(max, FontSize);
    ...
    private double FindBestWidthForLineFit(TextBlock control, double fontSize, double heightTarget, double min, double max)
    {
        if (control is null)
            return Math.Min(max, FontSize);
