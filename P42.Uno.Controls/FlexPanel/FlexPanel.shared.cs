...
public partial class FlexPanel : Panel
{
    ...
    {
        if (obj is FlexPanel flexPanel && flexPanel.root is { } root)
        {
    ...
    /// <see cref = "P:P42.Uno.Controls.FlexItem.AlignContent" /> can then be
    ...
    {
        if (d is FrameworkElement element && GetFlexItem(element) is { } item)
        {
    ...
    /// on a child element, the parent's value for <see cref="P:P42.Uno.Controls.FlexItem.AlignItems" />
    ...
    internal class FlexItem
    {
        ...
        private static void LayoutItem(FlexItem item, double width, double height)
        {
            ...
            for (var i = 0; i < item.Count; i++)
            {
                ...
                var mainAxisChildSize = child.Frame[layout.MainAxisSize];
                if (layout.Wrap)
                {
            ...
            if (layout.NeedLines && (layout.Lines?.Length ?? 0) > 0)
            {
                ...
                for (uint i = 0; i < (layout.Lines?.Length ?? 0); i++)
                {
                    ...
                    for (var j = line.ChildBegin; j < line.ChildEnd; j++)
                    {
        ...
        private static void LayoutItems(FlexItem item, int childBegin, int childEnd, int childrenCount, ref FlexLayout layout)
        {
            ...
            for (var i = childBegin; i < childEnd; i++)
            {
                ...
                // needed).
                var alignSize = child.Frame[layout.CrossAxisSize];
                var alignPos = layout.LineCrossAxisPosition + 0;
            ...
            if (layout.NeedLines)
            {
                ...
                ref var line = ref layout.Lines[layout.Lines.Length - 1];
