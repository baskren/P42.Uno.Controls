    ...
    private MenuFlyoutCell? SelectedCell
    {
        get
        {
            if (SelectedItem is { } item 
                && item.MenuFlyoutCellWeakRef is { } weakRef
                && weakRef.TryGetTarget(out MenuFlyoutCell? cell))
                    return cell;
            return null;
        }
    }
    ...
    private void CellTemplate_DataContextChanged(FrameworkElement sender, DataContextChangedEventArgs args)
    {
        ...
        if (args.NewValue is MenuItemBase baseItem)
        {
            ...
            if (baseItem.Text is string text)
                Children.Add(new TextBlock { Text = text }.Margin(5, 0).Column(1).CenterVt());
            if (args.NewValue is MenuGroup subItem)
                Children.Add(new TextBlock { Text = "\uE76C", FontFamily = new FontFamily("Segoe MDL2 Assets") }.Margin(5, 0).Column(3).CenterVt().Right());
        }
    }
