...
public partial class ContentAndDetailPresenter : Grid
{
    ...
    private Size ViewEstimatedSize
    {
        get
        {
    ...
    private void OpenDrawer(double percentOpen)
    {
        if (DrawerOrientation == Orientation.Horizontal)
        {
            ...
            SetRow(_detailDrawer, 0);
            SetRowSpan(_detailDrawer, 2);
            SetColumn(_detailDrawer, 1);
        ...
        else
        {
            ...
            SetRow(_detailDrawer, 2);
            SetRowSpan(_detailDrawer, 1);
            SetColumn(_detailDrawer, 0);
    ...
    private static object popupToDrawerResizeTrigger = new();

    private void LayoutDetailAndOverlay(Size size, double percentOpen, bool? knownDrawerMode = null)
    {
        if (size.IsZero())
            return;
    ...
    private bool LocalUpdateIsInDrawerMode(Size availableSize)
    {
        //if (Detail is null)
        //    return false;
    ...
    public async Task PopDetailAsync(bool animated = false)
    {
        ...
        if (drawerMode)
        {
            ...
            if (animated)
            {
                ...
                var animator = new ActionAnimator(0.89, 0.11, TimeSpan.FromMilliseconds(300), action);
                await animator.RunAsync();
            }
        }
        else
    ...
    private async void OnDismissPointerPressed(object sender, TappedRoutedEventArgs e)
    {
        if (PopOnPageOverlayTouch)
        {
            var dismissEventArgs = new DismissPointerPressedEventArgs();
